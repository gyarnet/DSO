...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh) {
    ...
    if (coarseTracker->firstCoarseRMSE < 0) coarseTracker->firstCoarseRMSE = achievedRes[0];  // 第一次跟踪的平均能量值
...
void FullSystem::printEigenValLine() {
    ...
    for (unsigned int i = 0; i < nsp.size(); i++) (*nullspacesLog) << nsp[i].dot(ef->lastHS * nsp[i]) << " " << nsp[i].dot(ef->lastbS) << " ";
    (*nullspacesLog) << "\n";
    nullspacesLog->flush();
}
